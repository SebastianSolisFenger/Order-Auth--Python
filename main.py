...
def showRemoveProductPanel():
    while True:
        ...
        if int(option) > 0 and int(option) <= cartFileLinesLength:
            cartFileLinesUpdated = (
                cartFileLines[: int(option) - 1] + cartFileLines[int(option) :]
            )
...
def showLoginPanel():
    ...
    for i in range(2):
        while True:
                ...
                for j in range(len(usersFileLines)):
                    if usersFileLines[j].startswith("--email"):
                        if usersFileLines[j].rstrip().split(" ")[1] == email:
                            correctPassword = (
                                usersFileLines[j + 1].rstrip().split(" ")[1]
                            )
            ...
            USER_FIRST_NAME = (
                usersFileLines[userLineIndexInUsersFile + 1].rstrip().split(" ")[1]
            )
            USER_LAST_NAME = (
                usersFileLines[userLineIndexInUsersFile + 2].rstrip().split(" ")[1]
            )
            USER_EMAIL = (
                usersFileLines[userLineIndexInUsersFile + 3].rstrip().split(" ")[1]
            )
            USER_PASSWORD = (
                usersFileLines[userLineIndexInUsersFile + 4].rstrip().split(" ")[1]
            )
